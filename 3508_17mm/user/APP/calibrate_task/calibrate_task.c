...
static void RC_cmd_to_calibrate(void);											//遥控器控制校准设备的校准
...
static gimbal_cali_t gimbal_cali;     //云台校准数据
 imu_cali_t gyro_cali;          //陀螺仪校准数据
static imu_cali_t accel_cali;         //加速度校准数据
static imu_cali_t mag_cali;           //磁力计校准数据

//static cali_sensor_t cali_sensor[CALI_LIST_LENGTH]; //校准设备数组，
cali_sensor_t cali_sensor[CALI_LIST_LENGTH]; //校准设备数组，

static const uint8_t cali_name[CALI_LIST_LENGTH][3] = {"HD", "GM", "GYR", "ACC", "MAG"}; //校准设备的名字
...
static void RC_cmd_to_calibrate(void)
{
    ...
    static uint16_t rc_cmd_time = 2005;
    static uint8_t rc_action_falg = 3;
...
//        rc_cmd_systemTick = xTaskGetTickCount();
				rc_cmd_systemTick = GetSysTime_us();//不确定是不是这么改，还没有验证过好使
        rc_action_falg = 1;
        rc_cmd_time = 0;
    }
...
//    calibrate_systemTick = xTaskGetTickCount();
		calibrate_systemTick = GetSysTime_us();//不确定是不是这么改，还没有验证过好使
