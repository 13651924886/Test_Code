...
#define FLASH_USER_ADDR ADDR_FLASH_SECTOR_23 //要写入flash扇区23，目前是最后一块，
...
{
    uint8_t name[3];    		//表头
    uint8_t cali_done;			//校准结束标志位
    uint8_t flash_len : 7;	//校准设备对应放入数据大小
    uint8_t cali_cmd : 1;   //校准指令标志位
    uint32_t *flash_buf;		//存放对应设备从FLASH读到的数据
